...
- **RPA Automation**: Scrapes active tickers from Yahoo Finance using Robot Framework.
...
- **Data Source**: We assume `yfinance` data is accurate. Missing values are handled by filling with 0, assuming that missing financial data often implies "negligible" or "not applicable" for that metric.
...
- `robots/`: Contains `tasks.robot` (The main RPA process).
- `src/`:
